...
from robot_utils import Recorder, get_arm_gripper_positions
...
def opening_ceremony(master_bot, puppet_bot):
    """ Move the 2 robots to a pose where it is easy to start demonstration """
...
def capture_one_episode(dt, max_timesteps, dataset_dir, dataset_name, overwrite):
    ...
    env = make_real_env(init_node=False, setup_robot=False)
...
def main(args):
    ...
    print(dataset_name + '\n')
    while True:
        is_healthy = capture_one_episode(DT, max_timesteps, dataset_dir, dataset_name, overwrite)
        if is_healthy:
            break
